...
let stevke b n =
let rec seznam_stevk b n acc1 = 
  match n / b with
  |0 ->  n :: acc1 
  |kolicnik -> seznam_stevk kolicnik b (( n mod b ) :: acc1) in
seznam_stevk  b n []  
...
let take n =
  if n < 0 then failwith "Ne moremo vzeti negativno število elementov"
  else
  let rec take1 n acc =
    function
    | [] -> List.rev acc
    | seznam when n = 0 -> List.rev acc
    | prvi :: ostalo -> take1 ( n - 1 ) ( prvi :: acc ) ostalo in
take1 n []
...
match seznam with
|[] -> []
| prvi :: ostalo -> if f prvi = true then drop_while f ostalo else seznam
...
let psi6 ( g, h) = fun x -> match x with
|In1 b -> g b
|In2 c -> h c
...
let pocisti seznam  = 
  let rec odstrani_nicle = function
  |[] -> []
  |0 :: ostalo -> odstrani_nicle ostalo
  |poljuben_seznam -> poljuben_seznam in
List.rev ( odstrani_nicle ( List.rev seznam ) )
...
let ( +++ ) polinom1 polinom2 = 
  ...
  | _, [] -> ( List.rev acc ) @ pol1
  | [], _ -> ( List.rev acc ) @ pol2
  | prvi1 :: ostalo1 , prvi2 :: ostalo2 ->  sestej ostalo1 ostalo2 ( ( prvi1 + prvi2 ) :: acc ) in
pocisti ( sestej polinom1 polinom2 [] )
...
let ( *** ) polinom1 polinom2 = 
let dolzina = List.length polinom1 in
let rec ponovi x n =
  if n <= 0 then []
  else x :: ( ponovi x ( n - 1 ) ) in
let rec zmnozi pol1 pol2 acc = match pol1 with
| [] -> acc
| prvi :: ostalo -> 
  zmnozi ostalo pol2 ( ( ( ponovi 0 ( dolzina -  ( List.length ostalo ) - 1  ) ) @ (List.map ( ( * ) prvi ) pol2) ) :: acc ) in
pocisti ( List.fold_left ( +++ ) [] ( zmnozi polinom1 polinom2 [] ) )
...
let rec vrednost polinom x =
let rec ugotovi_vrednost polinom x acc = match polinom with 
|[] -> acc
|prvi :: ostalo -> 
  let rec pomnozi a n acc1 = match n with
  |0 -> acc1
  |_ -> pomnozi a ( n - 1 ) ( a * acc1 ) in
  ugotovi_vrednost ostalo x ( ( prvi * ( pomnozi x ( List.length ostalo ) 1 ) ) + acc) in
ugotovi_vrednost (List.rev polinom) x 0
...
let odvod polinom =
let rec izracuna_odvod polinom acc =
  match polinom with
  | [] -> []
  | prvi :: [] -> acc
  | prvi :: ostalo -> izracuna_odvod ostalo (( prvi * ( List.length ostalo) ) :: acc) in
pocisti (izracuna_odvod (List.rev polinom) [])
...
let izpis polinom =
  let dolzina = List.length polinom in
  let rec naredi_izpis polinom acc =
    match polinom with
    |[] -> acc
    |prvi :: ostalo ->
      match prvi with
      | 0 -> naredi_izpis ostalo acc
      | a -> naredi_izpis ostalo ( " + " ^ ( string_of_int prvi) ^ " x \u{207" ^ ( string_of_int ( dolzina ) )  )
